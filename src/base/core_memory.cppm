...
export namespace base::mem {
  ...
  struct Arena {
     ...
     * @param zero_mem Set to true to zero-initialize the returned storage. Defaults to false.
     * @return Pointer to array storage, or nullptr on out-of-memory.
     * @pre init() has succeeded.
     */
    template <typename T>
    T* alloc_array(u64 count, bool zero_mem = false) {
     ...
     * @param zero_mem Forwards to alloc_array<T>(1, zero_mem); zero-initializes storage when true.
     * @return Pointer to allocated storage, or nullptr on failure.
     * @pre init() has succeeded.
     */
    template <typename T>
    T* alloc_struct(bool zero_mem = false) {
      return this->alloc_array<T>(1, zero_mem);
    }
  ...
  private:
     ...
     * @param zero_mem Set to true to zero allocated memory. Defaults to false.
     * @return Byte pointer to allocated span, or nullptr if capacity is exceeded.
     * @pre init() has succeeded.
     */
    u8* alloc_raw(u64 size, u64 alignment = 8, bool zero_mem = false) {
      BASE_ASSERT(this->buffer);
